"""Submit carbon-dioxide B3LYP/6-31G(d) gas-phase thermochemistry via Slurm."""
...
system = SystemQM(geometry="inputs/carbon_dioxide.xyz", charge=0, spin=0)
task = ThermoTask(
    system=system,
...
result = mae.run(
    rundir="runs/carbon_dioxide_thermo_work",
    task=task,
    engines=OrcaEngine(resources=Resources(cores=1, partition="32core_partition", time="24:00:00")),
)
print("carbon dioxide Gibbs free energy:", result.load("gibbs_free_energy"), result.unit("gibbs_free_energy"))
